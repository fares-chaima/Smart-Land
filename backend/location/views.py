...
RECOMMANDATIONS_CULTURE_WILAYA = {
...
def get_all_postes(request):
    try:
        ...
        for item in result:
            ...
            try:
                prix_num = float(prix) if prix not in [None, ''] else 0
                if prix_num < 0:
                    prix_num = 0
            except (TypeError, ValueError):
                prix_num = 0
            
            # Formatage du prix pour le frontend
            item['prix'] = {
                'raw': prix_num,
                'formatted': f"{prix_num:,.2f}"  # Format avec 2 décimales et séparateurs de milliers
            }

            # Nettoyage des autres champs avec valeurs par défaut
            item['titre'] = str(item.get('titre', 'Titre indisponible')).strip()
            item['location'] = str(item.get('location', 'Localisation inconnue')).strip()
            item['description'] = str(item.get('description', '')).strip()
            
            # Assurer que l'aire existe et est formatée correctement
            area = item.get('area', '0 ha')
            if not isinstance(area, str):
                area = f"{float(area)} ha" if area else '0 ha'
            item['area'] = area
...
def search_location(request):
    ...
    try:
        ...
        # Vérifie si query contient un mot-clé de culture
        for culture, wilayas in RECOMMANDATIONS_CULTURE_WILAYA.items():
            if culture in query:
                filtres.append({
                    "$or": [{"location": {"$regex": wilaya, "$options": "i"}} for wilaya in wilayas]
                })
                break  # On s'arrête à la première culture détectée

        # Vérifie si query contient un mot-clé de climat
        for climat, zones in RECOMMANDATIONS_CLIMAT_WILAYA.items():
            if climat in query:
                filtres.append({
                    "$or": [{"location": {"$regex": zone, "$options": "i"}} for zone in zones]
                })
                break  # On s'arrête à la première détection
        ...
        query_mongo = {"$or": filtres}
        results = list(collection.find(query_mongo, {"_id": 0}))

        if not results:
            return JsonResponse({"message": "Aucune location trouvée."}, status=404)
...
@csrf_exempt
def get_alll_postes(request):
    try:
        # Ignorer les deux premiers documents
        result = list(collection.find({}, {"_id": 0}).skip(7))
